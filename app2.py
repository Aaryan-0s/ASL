...
class SignLanguageProcessor(VideoProcessorBase):
    def __init__(self):
        ...
        self.blank_flag = 0
        self.word = ""
        self.sentence = ""  # Renamed from self.str to self.sentence
    ...
    def predict(self, test_image):
        ...
        # Secondary predictions for specific characters
        ...
        if self.ct[self.current_symbol] > 30:
            ...
            if self.current_symbol == 'blank':
                self.blank_flag += 1  # Increment the blank flag
                if self.blank_flag == 1 and self.word:
                    self.sentence += f" {self.word}"  # Add word to sentence (renamed from self.str)
                    self.word = ""  # Reset word
            else:
                self.blank_flag = 0  # Reset blank flag
                self.word += self.current_symbol  # Append symbol to word
...
if __name__ == "__main__":
    ...
    if webrtc_ctx.video_processor:
        ...
        while True:
            sentence_placeholder.text(f"Formed Sentence: {processor.sentence}")  # Updated from processor.str
